...
def register_jobs_tools(mcp: FastMCP):
    """Register Mattermost MCP jobs tools."""

    @mcp.tool(tags=["jobs"])
    async def mattermost_mcp_jobs(
        action: str = Field(
            description="Action to perform. Must be one of: 'get_jobs', 'create_job', 'get_job', 'download_job', 'cancel_job', 'get_jobs_by_type', 'update_job_status'"
        ),
        ...
        """Manage Mattermost MCP jobs operations."""
        if ctx:
            await ctx.info("Executing jobs operation: " + str(action) + "...")
        import json
