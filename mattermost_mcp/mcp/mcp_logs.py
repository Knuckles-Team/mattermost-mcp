...
def register_logs_tools(mcp: FastMCP):
    """Register Mattermost MCP logs tools."""

    @mcp.tool(tags=["logs"])
    async def mattermost_mcp_logs(
        action: str = Field(
            description="Action to perform. Must be one of: 'download_system_logs'"
        ),
        ...
        """Manage Mattermost MCP logs operations."""
        if ctx:
            await ctx.info("Executing logs operation: " + str(action) + "...")
        import json
