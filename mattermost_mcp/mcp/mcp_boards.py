...
def register_boards_tools(mcp: FastMCP):
    """Register Mattermost MCP boards tools."""

    @mcp.tool(tags=["boards"])
    async def mattermost_mcp_boards(
        action: str = Field(
            description="Action to perform. Must be one of: 'create_board'"
        ),
        ...
        """Manage Mattermost MCP boards operations."""
        if ctx:
            await ctx.info("Executing boards operation: " + str(action) + "...")
        import json
