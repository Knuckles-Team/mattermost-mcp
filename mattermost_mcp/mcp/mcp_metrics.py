...
def register_metrics_tools(mcp: FastMCP):
    """Register Mattermost MCP metrics tools."""

    @mcp.tool(tags=["metrics"])
    async def mattermost_mcp_metrics(
        action: str = Field(
            description="Action to perform. Must be one of: 'submit_performance_report'"
        ),
        ...
        """Manage Mattermost MCP metrics operations."""
        if ctx:
            await ctx.info("Executing metrics operation: " + str(action) + "...")
        import json
