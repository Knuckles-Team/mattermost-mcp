...
def register_roles_tools(mcp: FastMCP):
    """Register Mattermost MCP roles tools."""

    @mcp.tool(tags=["roles"])
    async def mattermost_mcp_roles(
        action: str = Field(
            description="Action to perform. Must be one of: 'get_all_roles', 'get_role', 'get_role_by_name', 'patch_role', 'get_roles_by_names'"
        ),
        ...
        """Manage Mattermost MCP roles operations."""
        if ctx:
            await ctx.info("Executing roles operation: " + str(action) + "...")
        import json
