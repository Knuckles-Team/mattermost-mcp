...
def register_actions_tools(mcp: FastMCP):
    """Register Mattermost MCP actions tools."""

    @mcp.tool(tags=["actions"])
    async def mattermost_mcp_actions(
        action: str = Field(
            description="Action to perform. Must be one of: 'open_interactive_dialog', 'submit_interactive_dialog', 'lookup_interactive_dialog'"
        ),
        ...
        """Manage Mattermost MCP actions operations."""
        if ctx:
            await ctx.info("Executing actions operation: " + str(action) + "...")
        import json
