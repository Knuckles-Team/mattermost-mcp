...
def register_reports_tools(mcp: FastMCP):
    """Register Mattermost MCP reports tools."""

    @mcp.tool(tags=["reports"])
    async def mattermost_mcp_reports(
        action: str = Field(
            description="Action to perform. Must be one of: 'get_users_for_reporting', 'get_user_count_for_reporting', 'start_batch_users_export', 'get_posts_for_reporting'"
        ),
        ...
        """Manage Mattermost MCP reports operations."""
        if ctx:
            await ctx.info("Executing reports operation: " + str(action) + "...")
        import json
